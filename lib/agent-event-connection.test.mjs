...
test("reports connection loss separately from agent completion", async () => {
  const { connection, sources, lifecycle } = createHarness();
  const ready = connection.ensureConnected("session-a");
  sources[0].open();
  sources[0].message({ type: "connected" });
  await ready;
  sources[0].error();
  assert.deepEqual(lifecycle, ["connecting", "connected", "disconnected"]);
});
