...
test("restores product run context from persisted specialist metadata", () => {
  const entries = [{
    type: "custom",
    customType: SUBAGENT_META_TYPE,
    data: {
      ...
      parentSessionId: "parent",
      parentSessionPath: "/tmp/parent.jsonl",
      product: { feature: "TK-TEST-001", runId: "RUN-001", role: "qa" },
      resourceSnapshot: {
        version: 1,
        appendSystemPrompt: ["Product context"],
        tools: ["read"],
        loadSkills: false,
        loadExtensions: false,
        product: { feature: "TK-TEST-001", runId: "RUN-001", role: "qa" },
  ...
  assert.deepEqual(readSubagentSessionResources(entries).product, {
    feature: "TK-TEST-001",
    runId: "RUN-001",
    role: "qa",
  });
  assert.equal(readSubagentRun(entries, "child", "/tmp/child.jsonl").product.role, "qa");
});
