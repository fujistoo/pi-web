...
function createFakeLaunchctl() {
  ...
  const calls = [];
  const targetFor = (args) => {
    const value = args.at(-1) ?? "";
    return value.includes("agent-worker") ? "worker" : "web";
  };
  const run = (args) => {
    calls.push(args);
    const target = targetFor(args);
    const current = states.get(target) ?? { loaded: false, running: false };
    if (args[0] === "print") {
      return {
        status: current.loaded ? 0 : 1,
        stdout: current.loaded && current.running ? "state = running\n" : "state = exited\n",
        stderr: current.loaded ? "" : "Could not find service\n",
      };
    }
    if (args[0] === "bootstrap" || args[0] === "kickstart") {
      states.set(target, { loaded: true, running: true });
      return { status: 0, stdout: "", stderr: "" };
    }
    if (args[0] === "bootout") {
      states.set(target, { loaded: false, running: false });
      return { status: 0, stdout: "", stderr: "" };
    }
...
test("serializes a valid, escaped LaunchAgent definition", () => {
  ...
  const workerPlist = createWorkerServicePlist({
    nodePath: "/usr/local/bin/node",
    workerScriptPath: "/Applications/Pi Web/bin/pi-web-agent-worker.js",
    packageDir: "/Applications/Pi Web",
    environment: { PI_WEB_PASSWORD: "secret" },
    paths: {
      stdoutPath: "/tmp/worker.log",
      stderrPath: "/tmp/worker.error.log",
    },
  });
  assert.match(workerPlist, new RegExp(`<string>${WORKER_SERVICE_LABEL}</string>`));
  assert.match(workerPlist, /<key>KeepAlive<\/key>\n  <false\/>/);
  assert.match(workerPlist, /pi-web-agent-worker\.js/);
});
...
test("start, status, and stop make idempotent launchctl decisions", () => {
  ...
  try {
    ...
    assert.deepEqual(fake.calls.map(([command]) => command), ["print", "bootstrap", "print", "bootstrap"]);
    assert.equal(fs.statSync(getServicePaths(root).plistPath).mode & 0o777, 0o600);
    assert.equal(fs.statSync(getWorkerServicePaths(root).plistPath).mode & 0o777, 0o600);
    ...
    assert.deepEqual(fake.calls.map(([command]) => command), [
      "print", "bootstrap", "print", "bootstrap", "print", "print",
    ]);
    ...
    assert.equal(reloadService(options).message, "Pi Web service reloaded.");
    assert.equal(restartService(options).message, "Pi Web service started.");
    ...
    assert.deepEqual(fake.calls.map(([command]) => command), [
      "print", "bootstrap", "print", "bootstrap", "print", "print",
      "print", "print", "print", "kickstart",
      "print", "bootout", "print", "print", "bootstrap",
      "print", "bootout", "print", "bootout", "print", "print",
    ]);
  } finally {
...
test("refuses to replace an unrelated LaunchAgent", () => {
  ...
  try {
    ...
    const fake = createFakeLaunchctl();
    const runLaunchctl = (args) => args[0] === "print" || args[2]?.includes("agent-worker")
      ? fake.run(args)
      : { status: 1, stdout: "", stderr: "" };
    assert.throws(
      () => startService(serviceOptions(root, runLaunchctl)),
