...
test("builds closed file mentions and quotes paths containing spaces", async () => {
  ...
  assert.equal(buildAtMentionText('project files/a "quote".md', false), '@"project files/a \\"quote\\".md" ');
  assert.equal(buildAtMentionText('quote".md', false), '@"quote\\".md" ');
  assert.equal(
...
test("builds and searches external-root entries by alias while retaining absolute paths", async () => {
  const {
    buildEntriesFromSearchRoot,
    fileEntryDisplayPath,
    filterFileEntries,
  } = await loadSubject();
  const entries = buildEntriesFromSearchRoot(
    ["components/ChatInput.tsx", "lib/file-fuzzy.ts"],
    "/opt/pi-web",
    "pi-web",
  );

  const [match] = filterFileEntries(entries, "pi-web/chatinput");
  assert.equal(match.path, "/opt/pi-web/components/ChatInput.tsx");
  assert.equal(match.rootAlias, "pi-web");
  assert.equal(match.relativePath, "components/ChatInput.tsx");
  assert.equal(fileEntryDisplayPath(match), "pi-web/components/ChatInput.tsx");
});
