...
test("does not advertise metadata-only ancestors as earlier messages", () => {
  const entries = [
    { id: "custom", parentId: null, type: "custom", timestamp: "t0" },
    { id: "model", parentId: "custom", type: "model_change", timestamp: "t1", provider: "test", modelId: "model" },
    { id: "thinking", parentId: "model", type: "thinking_level_change", timestamp: "t2", thinkingLevel: "off" },
    { id: "user", parentId: "thinking", type: "message", timestamp: "t3", message: { role: "user", content: "hello" } },
    { id: "assistant", parentId: "user", type: "message", timestamp: "t4", message: { role: "assistant", content: "reply" } },
  ];
  const context = buildSessionContext(entries, "assistant", { tail: 2 });
  assert.deepEqual(context.entryIds, ["user", "assistant"]);
