...
test("Agent tool description lists enabled effective profiles and refreshes when its factory reloads", async () => {
  ...
  assert.equal(first.executionMode, "parallel");
  assert.match(first.description, /Independent tasks must be emitted as separate Agent calls/);
  assert.match(first.promptGuidelines[1], /Hard rule/);
