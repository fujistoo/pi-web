...
test("discovers nested launcher sessions and keeps explicit relation kinds", async (t) => {
  const { dir, write } = fixture(t);
  const parent = write("parent");
  const nestedDir = join(dir, "launcher", "run-1");
  fs.mkdirSync(nestedDir, { recursive: true });
  const writeNested = (id, extraHeader = {}, entries = []) => {
    const path = join(nestedDir, `${id}.jsonl`);
    fs.writeFileSync(path, line({ type: "session", version: 3, id, cwd: dir, timestamp, ...extraHeader }) + entries.map(line).join(""));
    return path;
  };
  writeNested("child", { parentSession: parent }, [{
    type: "custom",
    customType: "pi-web:subagent",
    id: "meta",
    parentId: null,
    timestamp,
    data: { version: 1, parentSessionId: "parent", parentSessionPath: parent, profile: "Explore", description: "Nested child" },
  }]);
  writeNested("fork", { parentSession: parent });
  writeNested("parallel");

  const sessions = await listAllSessions({ force: true });
  assert.deepEqual(sessions.map((session) => session.id).sort(), ["child", "fork", "parallel", "parent"]);
  assert.deepEqual(sessions.find((session) => session.id === "child").relation, {
    kind: "subagent",
  ...
  assert.deepEqual(sessions.find((session) => session.id === "fork").relation, {
    kind: "fork",
    originSessionId: "parent",
  });
  assert.equal(sessions.find((session) => session.id === "parallel").parentSessionId, undefined);
});
