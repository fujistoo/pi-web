...
const { computeSessionTiming, computeSessionTotalActiveMs } = await jiti.import("./session-timing.ts");
...
function assistant(ms, content = []) {
  return entry("message", ms, { message: { role: "assistant", content } });
}
...
test("breaks active time into model, tools, and user tasks", () => {
  const entries = [
    message("user", 1000),
    assistant(4000, [{ type: "toolCall", toolCallId: "1" }]),
    message("toolResult", 7000),
    assistant(9000),
    message("user", 20000),
    assistant(23000),
  ];
  assert.deepEqual(computeSessionTiming(entries), {
    totalActiveMs: 11000,
    modelMs: 8000,
    toolMs: 3000,
    tasks: [
      { index: 1, entryId: entries[0].id, activeMs: 8000, modelMs: 5000, toolMs: 3000 },
      { index: 2, entryId: entries[4].id, activeMs: 3000, modelMs: 3000, toolMs: 0 },
    ],
  });
});
