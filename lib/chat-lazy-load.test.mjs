...
test("keeps loaded messages while appending a refreshed tail", async () => {
  const { mergeLoadedContext } = await loadSubject();
  const result = mergeLoadedContext(
    ["old", "kept"],
    ["old message", "stale kept message"],
    ["kept", "new"],
    ["fresh kept message", "new message"],
  );

  assert.deepEqual(result, {
    entryIds: ["old", "kept", "new"],
    messages: ["old message", "fresh kept message", "new message"],
  });
});
