...
test("recognizes pi-subagents run directories without reclassifying forks", () => {
  const root = join(tmpdir(), "sessions", "2026-09-12T04-04-48-399Z_parent-id");
  assert.equal(
    inferFleetParentSessionId(join(root, "child-id", "run-0", "session.jsonl")),
    "parent-id",
  );
  assert.equal(inferFleetParentSessionId(join(root, "forks", "fork.jsonl")), undefined);
  assert.equal(inferFleetParentSessionId(join(root, "child-id", "run-0", "not-session.jsonl")), undefined);
});
...
test("renders history and compaction in chronological order with aligned entry IDs", () => {
  ...
  assert.deepEqual(
    buildSessionContext(entries, "u3", { tail: 2 }).historyInputs.map(({ entryId, text }) => [entryId, text]),
    [
      ["u1", "old user request"],
      ["u2", "kept user request"],
      ["u3", "after compaction"],
    ],
    "the lightweight minimap index must not be truncated with the rendered page",
  );
});
...
test("returns an empty context for a null leaf", () => {
  for (const options of [{}, { tail: 50 }]) {
    ...
    assert.deepEqual(context.historyInputs, []);
...
test("groups a nameless pi-subagents run from its directory layout", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "pi-web-fleet-layout-"));
  const parentId = "fleet-layout-parent";
  const parentDir = join(dir, `2026-01-01T00-00-00-000Z_${parentId}`);
  const parentPath = `${parentDir}.jsonl`;
  const childPath = join(parentDir, "child-id", "run-0", "session.jsonl");
  mkdirSync(join(parentDir, "child-id", "run-0"), { recursive: true });
  writeFileSync(parentPath, `${JSON.stringify({
    type: "session", version: 3, id: parentId, timestamp: "2026-01-01T00:00:00.000Z", cwd: dir,
  })}\n`);
  writeFileSync(childPath, `${JSON.stringify({
    type: "session", version: 3, id: "fleet-layout-child", timestamp: "2026-01-01T00:00:00.000Z", cwd: dir,
  })}\n`);

  const originalListSessions = listSessions;
  listSessions = async () => [
    {
      path: parentPath, id: parentId, cwd: dir, created: new Date("2026-01-01T00:00:00.000Z"),
      modified: new Date("2026-01-01T00:00:00.000Z"), messageCount: 0, firstMessage: "(no messages)",
      allMessagesText: "",
    },
    {
      path: childPath, id: "fleet-layout-child", cwd: dir, created: new Date("2026-01-01T00:00:00.000Z"),
      modified: new Date("2026-01-01T00:00:01.000Z"), messageCount: 0, firstMessage: "(no messages)",
      allMessagesText: "",
    },
  ];
  resetSessionListState();
  t.after(() => {
    ...
    invalidateSessionPathCache("fleet-layout-child");
    resetSessionListState();
    rmSync(dir, { recursive: true, force: true });
  });

  const child = (await listAllSessions({ force: true })).find((session) => session.id === "fleet-layout-child");
