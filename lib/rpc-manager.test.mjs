...
test("running snapshots expose sessions with suppressed completion notifications", async () => {
  ...
  assert.match(runningRouteSource, /getCompletionNotificationSuppressedRpcSessionIds\(\)/);
  assert.match(sessionsRouteSource, /getCompletionNotificationSuppressedRpcSessionIds\(\)/);
});
...
test("normal session teardown paths use graceful extension shutdown", async () => {
  ...
  assert.match(deleteRouteSource, /sendExistingAgentWorkerCommand\(id, \{ type: "shutdown_session" \}\)/);
  assert.match(deleteRouteSource, /localSession\.shutdown\(\)/);
...
test("new-session route applies model scope during worker construction instead of follow-up commands", async () => {
  const source = await readFile(new URL("../lib/agent-worker-server.ts", import.meta.url), "utf8");
  const route = await readFile(new URL("../app/api/agent/new/route.ts", import.meta.url), "utf8");

  assert.match(source, /initialModel: \{ provider: provider as string, modelId: modelId as string \}/);
  assert.match(source, /thinkingLevel: explicitThinkingLevel/);
  assert.doesNotMatch(route, /set_model/);
  assert.doesNotMatch(route, /set_thinking_level/);
...
test("prompt routes mark only preflight failures as rejected", async () => {
  ...
  assert.match(existingRoute, /let promptAccepted = false/);
  assert.match(existingRoute, /sendAgentWorkerCommand\(id, body\)/);
  assert.match(existingRoute, /promptAccepted = body\.type === "prompt"/);
  assert.match(existingRoute, /commandType === "prompt" && !promptAccepted/);
  assert.match(newRoute, /let promptAccepted = false/);
  assert.match(newRoute, /createAgentWorkerSession\(body\)/);
  assert.match(newRoute, /promptAccepted = commandType === "prompt"/);
  assert.match(newRoute, /commandType === "prompt" && !promptAccepted/);
});
