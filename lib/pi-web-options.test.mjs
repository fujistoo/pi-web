...
const { getHelpText, parseCliArguments, parseLaunchOptions } = require("../bin/pi-web-options.js");
...
test("parses service commands while preserving the foreground default", () => {
  assert.equal(parseCliArguments([], {}).command, "foreground");
  assert.deepEqual(parseCliArguments(["start", "--port", "8080", "--no-open"], {}), {
    command: "start",
    help: false,
    port: "8080",
    hostname: "127.0.0.1",
  ...
  assert.equal(parseCliArguments(["stop"], {}).command, "stop");
  assert.equal(parseCliArguments(["status"], {}).command, "status");
  assert.equal(parseCliArguments(["restart"], {}).command, "restart");
  assert.equal(parseCliArguments(["reload"], {}).command, "reload");
  assert.equal(parseCliArguments(["update", "/tmp/pi web"], {}).sourceDir, "/tmp/pi web");
});

test("rejects unknown commands and missing update directories", () => {
  assert.throws(
    () => parseCliArguments(["unknown"], {}),
    /Unexpected argument/,
  );
  assert.throws(
    () => parseCliArguments(["update"], {}),
    /Unexpected argument/,
  );
});
