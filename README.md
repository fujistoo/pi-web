...
- **Two ways to branch**: **Branch in new chat** creates an independent session file from an earlier assistant message or selected text; **Edit from here** creates a branch inside the current session.
- **Project file tools**: browse and upload files, inspect Git diffs, and preview source, Markdown, images, audio, PDFs, and DOCX files with automatic refresh; large or truncated source files can opt into syntax highlighting.
